...
<<<<<<< HEAD
    public function generarJSONInvalidacion($idFactura, $idEvento) {
        ...
        if (!$encabezado) {
            return json_encode(["error" => "Factura no encontrada."]);
        }

        // Buscar el evento
        $sql_evento = "SELECT * FROM factura_evento WHERE id_evento = ?";
        $stmt = $this->db->prepare($sql_evento);
        ...
        if (!$evento) {
            return json_encode(["error" => "Evento de invalidación no encontrado."]);
        }

        // Estructura del Documento de Invalidación según MH
        $dteInvalidacion = [
            "identificacion" => [
                "version" => 2,
                "ambiente" => $encabezado['ambiente'],
                "codigoGeneracion" => strtoupper($evento['codigo_generacion_r']), // UUID del evento
                "fecAnula" => date('Y-m-d', strtotime($evento['fecha_evento'])),
                "horAnula" => date('H:i:s', strtotime($evento['fecha_evento']))
            ],
            "emisor" => [
                "nit" => $datosEmisor['nit'],
                "nombre" => $datosEmisor['nombre'],
                "tipoEstablecimiento" => $datosEmisor['tipo_establecimiento'],
                "nomEstablecimiento" => $datosEmisor['nombre_comercial'] ?: $datosEmisor['nombre'],
                "codigoEstablecimiento" => $datosEmisor['cod_estable_interno'] ?? "0001",
                "codigoPuntoVenta" => $datosEmisor['cod_punto_venta_int'] ?? "0001",
                "telefono" => $datosEmisor['telefono'],
                "correo" => $datosEmisor['correo']
            ],
            "documento" => [
                "tipoDte" => $encabezado['tipo_dte'],
                "codigoGeneracion" => strtoupper($encabezado['codigo_generacion']), // UUID original
                "selloRecibido" => $encabezado['sello_recibido'] ?? "SELLO_NO_RECIBIDO_EN_PRUEBAS",
                "numeroControl" => $encabezado['numero_control'],
                "fecEmi" => $encabezado['fecha_emision'],
                "montoIva" => (float)$encabezado['total_iva'],
                "codigoGeneracionR" => null,
                "tipoDocumento" => $encabezado['tipo_dte'],
                "fechaEmision" => $encabezado['fecha_emision']
            ],
            "motivo" => [
                "tipoAnulacion" => 2, // 2 = Anulación de un DTE
                "motivoAnulacion" => $evento['descripcion'],
                "nombreResponsable" => $evento['responsable'],
                "tipDocSolicita" => "36", // NIT por defecto
                "numDocSolicita" => $datosEmisor['nit'],
                "nombreSolicita" => $datosEmisor['nombre']
            ]
        ];
