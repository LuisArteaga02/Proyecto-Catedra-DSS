            ...
            <div class="ccf-container">
                ...
                <div class="ccf-header-banner">
                    ...
                    <div class="fe-header-right">
                        ...
                        <div>NRC: <strong style="color:#e8d800;"><?= htmlspecialchars($_SESSION['empresa_nrc'] ?? '') ?></strong></div>
                        <span class="fe-code" style="color:#f0a0a0;">MODELO: FACTURACIÓN PREVIA</span>
                    </div>
                </div>
                ...
                <div class="ccf-aviso">
                    ...
                    sobre la base neta: <strong>IVA = ventaGravada × 0.13</strong>. Son obligatorios el NIT, NRC,
                    actividad económica y nombre de la empresa receptora.
                </div>

                <form id="formCCF" action="procesar_factura.php" method="POST" novalidate>
                    ...
                    <section class="fe-section">
                        ...
                        <div class="box-info-ccf" style="margin-bottom:14px;">
                            ...
                            <div class="grid-3">
                                ...
                                <div class="fe-group">
                                    ...
                                    <input type="text" name="receptor_nrc" id="receptor_nrc"
                                           required maxlength="8" placeholder="00000000 (8 dígitos)">
                                ...
                                <div class="fe-group span2">
                                    ...
                                    <input type="text" name="receptor_desc_actividad" id="receptor_desc_actividad"
                                           required maxlength="150" placeholder="Descripción de la actividad económica">
                        ...
                        <div class="grid-3">
                            <div class="fe-group full">
                                ...
                                <input type="text" name="receptor_nombre" id="receptor_nombre"
                                       required maxlength="250" placeholder="Nombre completo o razón social de la empresa receptora">
                            ...
                            <div class="fe-group full">
                                ...
                                <input type="text" name="receptor_direccion" id="receptor_direccion"
                                       required maxlength="200" placeholder="Dirección completa de la empresa receptora">
                                <span class="error-msg" id="err_direccion">Ingrese la dirección del receptor.</span>
                            </div>
                            <div class="fe-group">
                                <label>ACTIVIDAD ECONÓMICA RECEPTOR</label>
                                <select name="actividad_receptor">
                                    <option value="">-- Seleccione --</option>
                                    <?php foreach ($actividades_db as $act): ?>
                                        <option value="<?= htmlspecialchars($act['codigo']) ?>">
                                            <?= htmlspecialchars($act['codigo']) ?> — <?= htmlspecialchars($act['descripcion']) ?>
                            ...
                            <div class="fe-group">
                                <label>NOMBRE COMERCIAL GENERAL</label>
                                <input type="text" name="receptor_nombre_com_gen"
                                       maxlength="150" placeholder="Nombre comercial si aplica">
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="fe-section">
                        <h3 class="section-title"><span class="num">3</span> Detalle de Productos / Servicios
                            <span style="font-size:9px;font-weight:500;color:#555;text-transform:none;margin-left:8px;">
                                Precios SIN IVA — distinto a FE
                            </span>
                        </h3>
                        <div class="ccf-aviso" style="margin-bottom:10px; border-radius: 4px; border-left: none;">
                            ℹ <strong>IVA por ítem</strong> = ventaGravada × 0.13 &nbsp;|&nbsp; <strong>Tributos por ítem</strong> = ["20"] — código IVA CAT-015
                        </div>
                        <div class="table-wrap">
                            <table class="items-table" id="tablaItems">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="th-iva">IVA (13%)</th>
                                        <th class="th-total">TOTAL C/IVA</th>
                        ...
                        <p style="font-size:10px;color:#888;margin-top:6px;font-style:italic;">
                            * El precio unitario se ingresa SIN IVA. El sistema calcula automáticamente el IVA (13%) y el Total con IVA.
                        </p>
                    </section>
                        ...
                        <div class="footer-grid">
                            ...
                            <div>
                                <div class="totales-box">
                                    ...
                                    <div class="total-row"><span>Ventas No Sujetas:</span><span id="t_nosujeta">0.00</span></div>
                                    <div class="total-row"><span>Ventas Exentas:</span><span id="t_exenta">0.00</span></div>
                                    <div class="total-row"><span>Total Ventas Gravadas (base):</span><span id="t_gravada">0.00</span></div>
                                    <div class="total-row separator"><span>Descuento Total:</span><span id="t_desc_global">-0.00</span></div>
                                    <div class="total-row"><span>Subtotal sin IVA:</span><span id="t_subtotal">0.00</span></div>
                                    <div class="total-row separator"><span>+ IVA 13% (tributo código 20):</span><span id="t_iva">0.00</span></div>
                                    <div class="total-row"><span>IVA Retenido / Retención Renta:</span><span id="t_retenciones">$0.00 / $0.00</span></div>
                    ...
                    <div class="form-actions">
                        ...
                        <button type="submit" class="btn-save" onclick="return validarFormulario()">📤 GENERAR CCF Y PDF</button>
                    </div>
...
(function () {
    ...
    document.getElementById('numero_control').placeholder = 'Generado al guardar (Ej: DTE-03...)';
})();
...
/* ================================================================
   CONSULTA NIT (simulada — conectar a Hacienda cuando disponible)
   ================================================================ */
function consultarNIT() {
    const nit = document.getElementById('receptor_nit').value.trim();
    if (!reNIT.test(nit)) {
        toast('Ingrese un NIT válido antes de consultar.', 'error');
        return;
    }
...
/* ================================================================
   AUTOCOMPLETAR DESCRIPCIÓN DE ACTIVIDAD
   ================================================================ */
document.getElementById('receptor_cod_actividad').addEventListener('change', function () {
    ...
    const desc = selected.split(' — ')[1] || '';
    document.getElementById('receptor_desc_actividad').value = desc;
});

/* ================================================================
   TABLA DE ÍTEMS (CCF — precios SIN IVA)
   ================================================================ */
function optsProductos() {
    let html = '<option value="">-- Seleccione un Producto --</option>';
    PRODUCTOS_DB.forEach(p => {
        // Para CCF el precio neto = precio / 1.13
        const precioNeto = (parseFloat(p.precio) / 1.13).toFixed(2);
        html += `<option value="${p.id_producto}" data-precio="${precioNeto}">${p.codigo_mh} - ${p.product_name}</option>`;
    });
    return html;
}
...
function filaItem(n) {
  ...
  <tr>
    ...
    <td><input type="number" name="precio_neto[]" class="in-table price" step="0.01" min="0" value="0.00" onkeypress="return isNum(event)" readonly></td>
    ...
    <td><input type="number" name="total_civa[]" class="in-table total_civa" step="0.01" value="0.00" readonly></td>
...
/* ================================================================
   CÁLCULO DE TOTALES — CCF (base neta, IVA separado)
   ================================================================ */
function calcTotales() {
    let grav = 0, exen = 0, nosuj = 0, desc = 0, iva = 0;
    ...
    // Retenciones ingresadas manualmente
    ...
    const total    = subtotal + iva - ivaRet - retRenta;
    ...
    document.getElementById('t_retenciones').textContent =
        `$${fmt(ivaRet)} / $${fmt(retRenta)}`;
    document.getElementById('t_total').textContent = '$ ' + fmt(total);
    document.getElementById('t_letras').textContent = 'SON: ' + numLetras(total);
    document.getElementById('t_credito_fiscal').textContent = '$' + fmt(iva);
